...
get_rainfall <- function(level = 'municipality', panel = FALSE){
  ...
  if(level == 'municipality'){
    ...
    dt <- read.xlsx('https://trello-attachments.s3.amazonaws.com/5ea83f462064047eed09f846/5ea83fc7d5707865983b4f14/2fe3829d60a70197d0b44337440348a2/PEECIPITACAO_1901_2019.xlsx')
  }
  if(level == 'state'){
    print('Please, wait for the data to download!')
    dt <- read.xlsx('https://trello-attachments.s3.amazonaws.com/5ea83f462064047eed09f846/5ea83fc7d5707865983b4f14/2fe3829d60a70197d0b44337440348a2/PEECIPITACAO_1901_2019.xlsx') %>%
      ...
      group_by(sigla.state, cod.state) %>% summarise_at(vars(5:123), funs(mean))
  }
  if(level == 'country'){
    print('Please, wait for the data to download!')
  ...
  if(panel == TRUE){
    dt<-dt %>% gather(v1, rainfall, 'PREC_1901':'PREC_2019') %>%
