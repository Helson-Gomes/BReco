get_rainfall <- function(level = 'municipality', panel = FALSE){
  # The code below help to avoid the note "no visible binding for global variable [variable name]"
  CD_GEOCMU <- cod.state <- sigla.state <- country <- v1 <- rainfall <- NULL
  temp <- tempfile()
  if(level == "municipality"){
    message("Please, wait for the data to download!")
    download.file("https://trello-attachments.s3.amazonaws.com/5ea83f462064047eed09f846/5ea83fc7d5707865983b4f14/2fe3829d60a70197d0b44337440348a2/PEECIPITACAO_1901_2019.xlsx", temp, quiet = T)
  ...
  if(panel == TRUE){
    dt <- tidyr::gather(dt, v1, rainfall, 'PREC_1901':'PREC_2019')
